...
def ch_number(cycles):
    with open('inproject_data.json', 'r') as f:
        data = json.load(f)

        chapter_number = data['chapter_number'] 
        
        updated_chapter_number = chapter_number + cycles

        data['chapter_number'] = updated_chapter_number

    with open('inproject_data.json', 'w') as f:
        json.dump(data, f)

    return updated_chapter_number
...
def preload_pages_same_tab(num):
    for _ in range(num):
        pyautogui.hotkey('shift', 'right')  # Move to the next page
        time.sleep(3)  # Wait for translation
    
    for _ in range(num):
        pyautogui.hotkey('shift', 'left')  # Move to the next page
        time.sleep(3)  # Wait for translation


def preload_pages_next_tab(num: int):
    for _ in range(num):
        pyautogui.hotkey('shift', 'right')  # Move to the next page
        time.sleep(3)  # Wait for translation
